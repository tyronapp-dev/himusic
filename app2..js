// ==========================================
// HIMUSIC CLOUD - MAIN APP SCRIPT
// ==========================================

import { auth } from "./firebase-config.js";
import { onAuthStateChanged } from "https://cdn.jsdelivr.net/npm/firebase@10.8.0/firebase-auth.js";

// --- START-LOGIK & FIREBASE SICHERHEIT ---
document.addEventListener('DOMContentLoaded', () => {
    // Firebase prüft, ob der Nutzer über login.html angemeldet wurde
...
async function fetchCoverFromiTunes(title, artist, retryCount = 0) {
    ...
    try {
        ...
        if (data.results && data.results.length > 0) {
            const coverUrl = data.results[0].artworkUrl100.replace('100x100bb.jpg', '600x600bb.jpg');
            return coverUrl;
        }
        return null;
    } catch (e) {
...
// ==========================================
// --- HAUPT-APP (GESCHÜTZT DURCH FIREBASE) ---
// ==========================================
function initApp() {

    // --- CLOUDFLARE API SETUP ---
    const API_URL = window.HiMusicConfig?.apiBaseUrl || "https://himusic-api.DEIN-NAME.workers.dev"; // Wird aus config.js gelesen
    ...
    async function apiDeleteSong(songId) {
        const response = await fetch(`${API_URL}/songs`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: [songId] }) });
    ...
    async function apiCreatePlaylist(name) {
        // Hinweis: Da wir in Cloudflare nur /playlists (GET) haben, müssten hier ggf. noch POST Routen im Worker ergänzt werden. 
        // Für diesen Code nehmen wir an, der Worker kann das.
    ...
    async function apiGetPlaylistSongs(playlistId) {
        // ACHTUNG: Hier ggf. Worker Route anpassen, wenn er /playlists/X/songs nicht hat
    ...
    console.log('Cloudflare API bereit!');
    ...
    function _doSavePlayerState() {
        const state = {
            ...
            queue: (window.playbackQueue || []).slice(0, 100),
        ...
        try { localStorage.setItem('himusic_state', JSON.stringify(state)); } catch(e) {}
    }

    function loadPlayerState() {
        try {
            const saved = localStorage.getItem('himusic_state');
            if(!saved) return;
            ...
            if (state.queue) window.playbackQueue = state.queue;
    ...
    window.togglePlaylistPlayback = async function(e, listId, songsArray = null) {
        ...
        window.playbackQueue = queueToPlay.slice(1);
    ...
    window.updateActiveHighlights = function() {
        ...
        if (bpHv) {
            if (activeVibes) {
                ...
                bpHv.innerText = _parseVibes(window.currentSongData.vibes).join(' • '); 
            } else {
                bpHv.innerText = "Aktueller Titel";
            }
        }
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden') {
            ...
            setTimeout(() => {
                if (memAudio && memAudio.paused && window._shouldBePlaying && memAudio.src) {
                    memAudio.play().catch(() => {});
                }
            }, 500);
        }
    ...
    setInterval(() => {
        if (memAudio && !memAudio.paused && memAudio.currentTime > 0) _doSavePlayerState();
    }, 5000);
    ...
    // ── HIMUSIC LOCAL STORE (IndexedDB) ──
    const HB_DB_NAME = 'HimusicAudio';
    ...
    function openHBDatabase() {
        return new Promise((resolve, reject) => {
            ...
            req.onupgradeneeded = (e) => {
                ...
                if (!db.objectStoreNames.contains(HB_STORE))
                    db.createObjectStore(HB_STORE, { keyPath: 'fileUrl' });
            };
    ...
    async function downloadToLocal(fileUrl, title) {
        try {
            ...
            if (song) {
                document.querySelectorAll(`.song-item[data-id="${song.id}"] .offline-badge`)
                    .forEach(b => b.style.display = 'inline');
            }
        } catch(e) {}
    }
    ...
    setTimeout(async () => {
        ...
        if (offlineBtn) {
            ...
            const validOfflineCount = window.globalSongsData
                ? window.globalSongsData.filter(s => s.file_url && localUrls.has(s.file_url)).length
                : localUrls.size;
    ...
    window.playSong = function(title, artist, coverUrl, fileUrl) {
        ...
        if (!_skipNextHistoryPush && _oldSongData && _oldSongData.fileUrl && _oldSongData.fileUrl !== fileUrl) {
            if (window.playbackHistory.length === 0 || window.playbackHistory[window.playbackHistory.length - 1]?.fileUrl !== _oldSongData.fileUrl) {
                window.playbackHistory.push(_oldSongData);
            }
            if (window.playbackHistory.length > 100) window.playbackHistory.shift();
        }
        ...
        window.currentSongData = { 
            ...
            duration: window.currentSongDuration, vibes: _parseVibes(foundSong?.vibes)
        };
        ...
        if ('mediaSession' in navigator) {
            navigator.mediaSession.setActionHandler('play', async () => {
                if (audioPlayer.src && audioPlayer.src !== window.location.href) {
                    try { await audioPlayer.play(); } catch(err) {}
                } else if (window.currentSongData) {
                    const s = window.currentSongData;
                    window.playSong(s.title, s.artist, s.cover_data || s.coverUrl, s.file_url || s.fileUrl);
                }
            });
        ...
        getLocalAudio(fileUrl).then(localUrl => {
            if (localUrl) {
                if (!audioPlayer.paused) {
                    ...
                    audioPlayer.load();
                    audioPlayer.addEventListener('canplay', function swap() {
                        audioPlayer.currentTime = t;
                        audioPlayer.play().catch(() => {});
                        audioPlayer.removeEventListener('canplay', swap);
                    });
                } else {
                    audioPlayer.src = localUrl;
                    audioPlayer.load();
    ...
    window.playNextSong = function() {
        ...
        if (!window.playbackQueue || window.playbackQueue.length === 0) {
            if (window.globalSongsData && window.globalSongsData.length > 0) {
                window.playbackQueue = [...window.globalSongsData].sort(() => 0.5 - Math.random());
            } else return;
        }
        const nextSong = window.playbackQueue.shift();
    ...
    window.playPrevSong = function() {
        ...
        if (isDoubleTap) {
            isChangingSong = true;
            setTimeout(() => isChangingSong = false, 800);
            if (window.playbackHistory && window.playbackHistory.length > 0) {
                const prevSong = window.playbackHistory.pop();
                if (window.currentSongData) window.playbackQueue.unshift(window.currentSongData);
    ...
    audioPlayer.addEventListener('ended', () => {
        ...
        if (btnRepeat && btnRepeat.classList.contains('ctrl-active')) {
            audioPlayer.currentTime = 0; audioPlayer.play();
        } else {
            window.playNextSong();
        }
    ...
    function _countAvailableSelectedSongs() {
        ...
        selectedSongs.forEach(songId => {
            if (!_existingPlaylistIds.has(String(songId))) {
                count++;
            }
        });
        return count;
    ...
    function lazyRenderBatch() {
        ...
        for (let i = lazyRendered; i < end; i++) {
            ...
            window.allSongsElements.push(div);
        }
        ...
        if (lazyRendered < lazyAllSongs.length) {
            ...
            lazyObserver = new IntersectionObserver(entries => {
                if (entries[0].isIntersecting) lazyRenderBatch();
            }, { rootMargin: '500px' });
    ...
    const SONGS_CACHE_KEY = 'himusic_songs_snapshot';
    const SONGS_CACHE_TS  = 'himusic_songs_snapshot_ts';

    function saveSongsSnapshot(songs) {
        try {
            localStorage.setItem(SONGS_CACHE_KEY, JSON.stringify(songs));
            localStorage.setItem(SONGS_CACHE_TS, Date.now().toString());
        } catch(e) {
            try {
                const slim = songs.map(s => ({ ...s, cover_data: s.cover_data?.startsWith('http') ? s.cover_data : '' }));
                localStorage.setItem(SONGS_CACHE_KEY, JSON.stringify(slim));
                localStorage.setItem(SONGS_CACHE_TS, Date.now().toString());
            } catch(e2) {}
        }
    }

    function loadSongsSnapshot() {
        try {
    ...
    function updateSongDOM(songDiv, song, playlistSongId = null) {
        ...
        songDiv.addEventListener('touchstart', (e) => {
            songStartX = e.touches[0].clientX;
            isSwiping = false;
        }, {passive: true});
        ...
        songDiv.addEventListener('touchend', (e) => {
            ...
            if (Math.abs(diffX) > 60) {
                if (diffX < 0) { 
                    window.playbackQueue.unshift(song); 
                    savePlayerState();
        ...
        songDiv.addEventListener('click', (e) => {
            ...
            if (e.target.closest('.song-context-btn')) {
                ...
                document.getElementById('song-context-overlay')?.classList.add('active');
                return;
            }
            if (currentMode !== 'normal' && currentMode !== 'reorder') {
                const checkbox = songDiv.querySelector('.song-checkbox');
                if (checkbox.classList.toggle('checked')) selectedSongs.add(String(song.id));
                else selectedSongs.delete(String(song.id));
                const selCount = document.getElementById('sel-count');
                const count = _countAvailableSelectedSongs();
                if(selCount) selCount.innerText = `${count} ausgewählt`;
                ...
                if (window.currentOpenPlaylistId && window.currentPlaylistSongs) {
                    ...
                    if (songIndex > -1) {
                        window.playbackQueue = window.currentPlaylistSongs.slice(songIndex + 1);
                    }
                    ...
                    window.playbackQueue = otherSongs.sort(() => 0.5 - Math.random());
                }
                savePlayerState(); 
            }
        });
    ...
    const UPLOAD_CONFIG = {
        PARALLEL_UPLOADS: 1, 
        MAX_RETRIES: 2,
        WORKER_URL: `${API_URL}/upload`, // Verwendet deine definierte API
        ITUNES_TIMEOUT: 6000
    };

    let _uploadQueue = [];
    let _uploadRunning = false;
    let _uploadStats = { total: 0, success: 0, failed: 0, skipped: 0 };
    let _inFlightKeys = new Set();
    let _sessionUploadedKeys = new Set();

    async function _isDuplicate(file) {
        // Pseudo Fingerprint Logic placeholder
        return { isDup: false, hash: file.name };
    }

    function _saveFingerprint(hash, id) {}
        ...
        try {
            ...
            try {
                const tags = await new Promise((resolve, reject) => {
                    jsmediatags.read(file, { onSuccess: (t) => resolve(t.tags), onError: reject });
                });
                if (tags.title) title = tags.title.trim();
                if (tags.artist) artist = tags.artist.trim();
            } catch (e) {
                console.warn("Tags konnten nicht gelesen werden, nutze Dateinamen.");
            }

            const isDup = window.globalSongsData.some(s => 
                (s.title === title && s.artist === artist) || 
                (s.file_size === file.size && s.title === title)
            );

            if (isDup) {
                _uploadStats.skipped++;
                return { success: false, skipped: true };
            }

            const safeFileName = `${Date.now()}_${Math.random().toString(36).substr(2, 5)}_${file.name.replace(/[^a-zA-Z0-9.]/g, '_')}`;
            const uploadResponse = await fetch(`${UPLOAD_CONFIG.WORKER_URL}/${safeFileName}`, {
                method: 'PUT',
                body: file,
            ...
            if (retryCount < UPLOAD_CONFIG.MAX_RETRIES) {
                await new Promise(resolve => setTimeout(resolve, 1000));
                return _uploadSingleFile(file, retryCount + 1);
            }
            _uploadStats.failed++;
    ...
    async function _processUploadQueue() {
        ...
        _uploadRunning = true;
        _uploadStats = { total: _uploadQueue.length, success: 0, failed: 0, skipped: 0 };
        ...
        try {
            ...
            console.error('Kritischer Fehler in Upload-Queue:', err);
    ...
    if (btnAddSongs && fileUploadInput) {
        btnAddSongs.addEventListener('click', () => {
        ...
        fileUploadInput.addEventListener('change', async (e) => {
            ...
            btnAddSongs.innerHTML = `⏳ ${_uploadQueue.length} Songs in Queue...`;

            if (!_uploadRunning) _processUploadQueue();
        });
    }

    // --- FEHLENDE SONGS FINDEN ---
    document.getElementById('action-find-missing')?.addEventListener('click', () => {
        actionSheetOverlay.classList.remove('active');
        setTimeout(() => document.getElementById('missing-songs-input')?.click(), 300);
    });
    ...
    document.getElementById('missing-songs-input')?.addEventListener('change', async (e) => {
        ...
        listEl.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-secondary);">⏳ Vergleiche mit Datenbank...</div>';
        titleEl.innerText = 'Analysiere...';
        ...
        if (missing.length === 0) {
            listEl.innerHTML = '<div style="text-align:center;padding:40px 20px;color:#30d158;">✅ Alle Songs sind bereits hochgeladen!</div>';
            e.target.value = '';
            return;
        }
        ...
        missing.forEach((file, idx) => {
            ...
            div.innerHTML = `
                <div style="width:36px;height:36px;border-radius:8px;background:rgba(255,159,10,0.15);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ff9f0a" stroke-width="2"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
                </div>
                <div style="flex:1;min-width:0;">
                    <div style="font-size:14px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${name}</div>
                    <div style="font-size:12px;color:var(--text-secondary);">${sizeMB} MB</div>
                </div>
            `;
        ...
        uploadAllBtn.onclick = async () => {
            ...
            uploadAllBtn.innerText = '⏳ Lädt hoch...';
            ...
            for (const file of missing) {
                ...
                try {
                    ...
                    if (!uploadResponse.ok) throw new Error(`Upload fehlgeschlagen: ${uploadResponse.status}`);
                    ...
                    await new Promise((resolve) => {
                        jsmediatags.read(file, {
                            onSuccess: async (tag) => {
                                const tags = tag.tags;
                                const title = tags.title?.trim() || fallbackName;
                                const artist = tags.artist?.trim() || 'Unbekannter Künstler';
                                let finalCoverUrl = await fetchCoverFromiTunes(title, artist);
                                await apiCreateSong({ 
                                    title, artist,
                                    cover_data: finalCoverUrl || '', 
    ...
    document.getElementById('missing-songs-close')?.addEventListener('click', () => {
        document.getElementById('missing-songs-overlay').classList.remove('active');
    });
    ...
    if(ctxDelete) {
        ctxDelete.addEventListener('click', () => {
            songContextOverlay.classList.remove('active');
            selectedSongs.clear();
            selectedSongs.add(String(window.currentContextSongId));
    ...
    if(ctxRemoveFromPlaylist) {
        ctxRemoveFromPlaylist.addEventListener('click', async () => {
            songContextOverlay.classList.remove('active');
            try {
                await apiRemoveSongFromPlaylist(window.currentOpenPlaylistId, window.currentContextSongId);
                const el = document.querySelector(`#playlist-details-songs-container .song-item[data-id="${window.currentContextSongId}"]`);
                if(el) el.remove();
                window.fetchPlaylistsForPage(true);
            } catch (error) {
   ...
   if(ctxCreateStation) {
        ctxCreateStation.addEventListener('click', () => {
            ...
            const savedStations = JSON.parse(localStorage.getItem('himusic_stations') || '[]');
            savedStations.unshift(newStation); 
            localStorage.setItem('himusic_stations', JSON.stringify(savedStations));
    ...
    function _savePendingEdit(songId, changes) {
        const pending = JSON.parse(localStorage.getItem('himusic_pending_edits') || '[]');
        ...
        localStorage.setItem('himusic_pending_edits', JSON.stringify(pending));
    }
    ...
    async function _flushPendingEdits() {
        ...
        for (const entry of pending) {
            try {
                await apiUpdateSong(entry.id, entry.changes);
                flushed++;
            } catch (error) {
                console.error('Fehler beim Sync:', error);
            }
        }
        if (flushed > 0) { localStorage.removeItem('himusic_pending_edits'); fetchSongsFromDatabase(true); }
    }
    ...
    if (btnSaveTags) {
        btnSaveTags.addEventListener('click', async () => {
            ...
            try {
                    ...
                    if (window.currentPlayingSongId == songId) {
                        ...
                        if ('mediaSession' in navigator && navigator.mediaSession.metadata) {
                            navigator.mediaSession.metadata = new MediaMetadata({
                                ...
                                album: 'Himusic Cloud',
                                artwork: changes.cover_data ? [
                                    { src: changes.cover_data, sizes: '512x512', type: 'image/jpeg' }
                                ] : []
                            });
                        }
                    }
                ...
                if (typeof window.updateActiveHighlights === 'function') window.updateActiveHighlights();
                fetchSongsFromDatabase(true);
                _showToast('✅ Gespeichert');
            } catch (error) {
                _savePendingEdit(window.currentContextSongId, changes);
                _showToast('⚠️ Fehler – lokal gesichert');
            }
        });
    }

    // --- SONGS LÖSCHEN & SORTIEREN ---
    ...
    document.getElementById('sel-all')?.addEventListener('click', () => {
        ...
        if (allSelected) {
            selectableSongs.forEach(el => {
                el.querySelector('.song-checkbox')?.classList.remove('checked');
                selectedSongs.delete(el.dataset.id);
            });
            if (_currentFilteredSongs) {
                _currentFilteredSongs.forEach(s => { if (!_existingPlaylistIds.has(String(s.id))) selectedSongs.delete(String(s.id)); });
            }
        } else {
            selectableSongs.forEach(el => {
            ...
            sourceList.forEach(s => {
                if (!_existingPlaylistIds.has(String(s.id))) selectedSongs.add(String(s.id));
            });
        }

        const countEl = document.getElementById('sel-count');
        const count = _countAvailableSelectedSongs();
        if (countEl) countEl.innerText = `${count} ausgewählt`;
    });
    ...
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                window.allSongsElements.forEach(el => { 
                    if(idsToDelete.includes(el.dataset.id)) el.remove(); 
                });
                const confirmOverlay = document.getElementById('confirm-dialog-overlay');
    ...
    document.querySelectorAll('#sort-sheet-overlay .sort-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (window.currentSortTarget === 'playlist') {
                ...
                targetElements = window.allSongsElements;
            }
    ...
    document.getElementById('action-filter-no-vibe')?.addEventListener('click', async () => {
        ...
        if (_noVibeFilterActive) {
            _noVibeFilterActive = false;
            songsContainer.innerHTML = '';
            window.allSongsElements.forEach(el => songsContainer.appendChild(el));
            return;
        }

        _noVibeFilterActive = true;
        songsContainer.innerHTML = '<div style="text-align:center; padding: 20px; color: var(--text-secondary);">Lade Songs ohne Vibe...</div>';

        const noVibeSongs = window.globalSongsData.filter(song => {
            let vibes = song.vibes;
            if (typeof vibes === 'string') {
                try { vibes = JSON.parse(vibes); } catch(e) { vibes = []; }
            }
            if (!Array.isArray(vibes)) vibes = [];
            return vibes.length === 0;
        });

        if (noVibeSongs.length === 0) {
            songsContainer.innerHTML = '<div style="text-align:center; padding: 40px 20px; color: var(--text-secondary);">Alle Songs haben bereits einen Vibe 🎉</div>';
            return;
        }

        songsContainer.innerHTML = '';
        noVibeSongs.forEach(song => {
            const existing = window.allSongsElements.find(el => parseInt(el.dataset.id) === song.id);
            if (existing) {
                songsContainer.appendChild(existing);
            } else {
                const div = document.createElement('div');
                div.className = 'song-item';
                div.dataset.id = song.id;
    ...
    if(actionFilterVibeBtn && vibeFilterOverlay) {
        actionFilterVibeBtn.addEventListener('click', () => {
            ...
            if(filterVibesContainer.innerHTML === '') {
                const noVibePill = document.createElement('div');
                noVibePill.className = 'vibe-pill';
                noVibePill.innerText = 'Kein Vibe';
                noVibePill.dataset.vibe = '__no_vibe__';
                noVibePill.addEventListener('click', () => noVibePill.classList.toggle('active'));
                filterVibesContainer.appendChild(noVibePill);
    ...
    if(btnApplyVibeFilter) {
        btnApplyVibeFilter.addEventListener('click', () => {
            ...
            if(selectedFilterVibes.length === 0) {
                window.allSongsElements = [];
                lazyRendered = 0;
                _currentFilteredSongs = null;
                lazyRenderBatch();
                vibeFilterOverlay.classList.remove('active');
                return;
            }

            const filterNoVibe = selectedFilterVibes.includes('__no_vibe__');
            const realVibes = selectedFilterVibes.filter(v => v !== '__no_vibe__');

            const matched = lazyAllSongs.filter(song => {
                let vibes = song.vibes;
                if (typeof vibes === 'string') { try { vibes = JSON.parse(vibes); } catch(e) { vibes = []; } }
                if (!Array.isArray(vibes)) vibes = [];
                const clean = vibes.filter(v => v && v.toString().trim() !== '');
                const hasNoVibes = clean.length === 0;
                const hasRealVibe = realVibes.length > 0 && clean.some(v => realVibes.includes(v));
                return (filterNoVibe && hasNoVibes) || hasRealVibe;
            });

            if (matched.length === 0) {
                songsContainer.innerHTML = '<div style="text-align:center;padding:40px 20px;color:var(--text-secondary);">Keine Songs mit diesem Vibe gefunden.</div>';
                vibeFilterOverlay.classList.remove('active');
                return;
            }
            ...
            matched.forEach(song => {
                let el = window.allSongsElements.find(e => parseInt(e.dataset.id) === song.id);
                if (!el) {
                    el = document.createElement('div');
                    el.className = 'song-item';
                    el.dataset.id = song.id;
                    updateSongDOM(el, song);
                    window.allSongsElements.push(el);
                }
                frag.appendChild(el);
            });
            songsContainer.appendChild(frag);
            _currentFilteredSongs = matched; 
            vibeFilterOverlay.classList.remove('active');
        });
    }
    ...
    window.fetchPlaylistsForPage = async function(force = false) {
        const cachedPlaylists = (() => { try { const r = localStorage.getItem('himusic_playlists_snapshot'); return r ? JSON.parse(r) : null; } catch(e) { return null; } })();
        
        if (cachedPlaylists && cachedPlaylists.length > 0) {
            window.globalPlaylistsData = cachedPlaylists;
            if (typeof window.renderHomeSections === 'function') window.renderHomeSections();
            _renderPlaylistsUI(cachedPlaylists, []);
        }

        const now = Date.now();
        if (_fetchPlaylistsRunning) return;
        if (!force && cachedPlaylists && (now - _playlistsLastFetched) < 60000) return;
        _fetchPlaylistsRunning = true;

        try {
            const playlists = await apiGetAllPlaylists();
            window.globalPlaylistsData = playlists;
            _playlistsLastFetched = Date.now();
            
            try { localStorage.setItem('himusic_playlists_snapshot', JSON.stringify(playlists)); } catch(e) {}
    ...
    function _renderPlaylistsUI(playlists, allPlaylistSongs) {
        ...
        const durMap = {};
        if (window.globalSongsData) window.globalSongsData.forEach(s => { durMap[s.id] = s.duration || 0; });
        
        const playlistSongSets = {};
        allPlaylistSongs.forEach(ps => {
            if (!playlistSongSets[ps.playlist_id]) playlistSongSets[ps.playlist_id] = new Set();
            playlistSongSets[ps.playlist_id].add(ps.song_id);
        });
        const playlistSongCountMap = {};
        Object.keys(playlistSongSets).forEach(pid => {
            playlistSongCountMap[pid] = playlistSongSets[pid].size;
        });
        ...
        if (playlistsPageContainer) {
            ...
            if (playlists.length === 0) {
                ...
                playlists.forEach(playlist => {
                    ...
                    let coverHtml = `
                        <div class="song-cover" style="${bgStyle} border-radius: 6px;">
                            ${innerSvg}
                        </div>
                    `;

                    let count = playlistSongCountMap[playlist.id] || 0;
                    let dur = 0;
                    allPlaylistSongs.forEach(ps => { if (ps.playlist_id == playlist.id) dur += durMap[ps.song_id] || 0; });
    ...
    window.openPlaylistSelection = function() {
        if (!playlistSelectionOverlay) return;
        playlistSelectionOverlay.classList.add('active');
        const stale = (Date.now() - _playlistsLastFetched) > 60000;
        if (!window.globalPlaylistsData || window.globalPlaylistsData.length === 0 || stale) {
            window.fetchPlaylistsForPage(true);
        }
    };
    ...
    async function createNewPlaylistProcess() {
        ...
        try {
            ...
            if (newPlaylist && (selectedSongs.size > 0 || window.currentContextSongId)) {
                addSelectedSongsToPlaylist(newPlaylist.id, newPlaylist.name);
            }
        } catch (error) {
    ...
    async function addSelectedSongsToPlaylist(playlistId, playlistName) {
        ...
        try {
            ...
            if (newIds.length === 0) {
                _showToast('Alle ausgewählten Songs sind bereits in der Playlist');
                document.getElementById('sel-cancel')?.click();
                return;
            }
            ...
            localStorage.removeItem(`himusic_playlist_${playlistId}_songs`);
            localStorage.removeItem(`himusic_playlist_${playlistId}_ts`);

            try {
                const snap = JSON.parse(localStorage.getItem('himusic_ps_snapshot') || '[]');
                const newEntries = newIds.map(songId => ({ playlist_id: playlistId, song_id: parseInt(songId) }));
                localStorage.setItem('himusic_ps_snapshot', JSON.stringify([...snap, ...newEntries]));
            } catch(e) {
                localStorage.removeItem('himusic_ps_snapshot');
            }
            _existingPlaylistIds = new Set();
            ...
            setTimeout(() => {
                ...
                if (typeof window.openPlaylistDetails === 'function') {
                    window.openPlaylistDetails(playlistId, playlistName, true);
                }
            }, 300);
      } catch (error) {
    ...
    if (confirmDeletePlaylistBtn) {
        confirmDeletePlaylistBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if(typeof endPlaylistSelectionMode === 'function') endPlaylistSelectionMode();
            } catch (error) {
                alert("Fehler beim Löschen: " + error.message);
            }
    ...
    document.getElementById('btn-save-playlist')?.addEventListener('click', async () => {
        ...
        try {
            await apiUpdatePlaylist(window.currentContextPlaylistId, { 
                name: newName, 
                cover_data: currentEditPlaylistCoverData 
            });
    ...
    document.getElementById('ctx-pl-add-queue')?.addEventListener('click', async () => {
        ...
        try {
            const songs = await apiGetPlaylistSongs(window.currentContextPlaylistId);
            ...
            let songsInPl = songs.filter(s => s !== null);
            songsInPl = songsInPl.sort(() => 0.5 - Math.random()); 
            
            window.playbackQueue.push(...songsInPl); 
    ...
    document.getElementById('ctx-pl-add-songs')?.addEventListener('click', async () => {
        ...
        try {
            const playlistSongs = await apiGetPlaylistSongs(window.currentContextPlaylistId);
            const existingIds = playlistSongs.map(s => s.id);
            ...
            selectedSongs.clear();
            songsContainer.classList.add('selection-mode');
            _existingPlaylistIds = new Set(existingIds.map(id => String(id)));
    ...
    window.renderHomeSections = function() {
        const recentId = localStorage.getItem('himusic_last_playlist');
        ...
        if(mixContainer) {
            let mixes = JSON.parse(localStorage.getItem('himusic_vibe_mixes') || '[]');
            const now = Date.now();
            mixes = mixes.filter(m => m.expires > now); 
            localStorage.setItem('himusic_vibe_mixes', JSON.stringify(mixes));
            ...
            if(mixes.length === 0) {
                ...
                mixes.forEach(mix => {
                    ...
                    if (playBtn) playBtn.addEventListener('click', (e) => {
                        const ids = mix.songIds || (mix.songs || []).map(s => s.id);
                        const songs = ids.map(id => window._songIndex?.get(id)).filter(Boolean);
                        const shuffled = [...songs].sort(() => Math.random() - 0.5);
                        window.togglePlaylistPlayback(e, mix.id, shuffled);
                    });
                    ...
                    if (typeof addLongPressListener === 'function') {
                        addLongPressListener(card, (e) => {
                            e.preventDefault(); e.stopPropagation();
                            if (confirm(`"${mix.name}" löschen?`)) {
                                let saved = JSON.parse(localStorage.getItem('himusic_vibe_mixes') || '[]');
                                saved = saved.filter(m => m.id !== mix.id);
                                localStorage.setItem('himusic_vibe_mixes', JSON.stringify(saved));
        ...
        if(stationsContainer) {
            let stations = JSON.parse(localStorage.getItem('himusic_stations') || '[]');
            const now = Date.now();
            stations = stations.filter(s => s.expires > now); 
            localStorage.setItem('himusic_stations', JSON.stringify(stations));
                ...
                stations.forEach(station => {
                    ...
                    card.addEventListener('click', () => {
                        ...
                        if (station.songs.length > 0) {
                            ...
                            window.playbackQueue = station.songs.slice(1);
                            savePlayerState();
                        }
                    });
    ...
    if(homeSearchInput) {
        homeSearchInput.addEventListener('input', debounce((e) => {
            ...
            if (query === '') {
                ...
                const matchedSongs = window.globalSongsData.filter(song => 
    ...
    if (songsSearchInput) {
        songsSearchInput.addEventListener('input', debounce((e) => {
            ...
            if (matched.length === 0) {
                songsContainer.innerHTML = '<div style="text-align:center;padding:40px 20px;color:var(--text-secondary);">Keine Songs gefunden.</div>';
                return;
            }
   ...
   document.getElementById('btn-home-random')?.addEventListener('click', () => {
        ...
        const shuffled = [...window.globalSongsData].sort(() => 0.5 - Math.random());
        const first = shuffled[0];
        window.playSong(first.title, first.artist, first.cover_data, first.file_url);
        window.playbackQueue = shuffled.slice(1);
        savePlayerState();
    });
    ...
    document.getElementById('btn-home-vibemix')?.addEventListener('click', () => {
        ...
        if(cont && cont.innerHTML === '') {
            ...
            noVibePill.innerText = '🚫 Kein Vibe';
            noVibePill.dataset.vibe = '__no_vibe__';
            noVibePill.addEventListener('click', () => {
                noVibePill.classList.toggle('active');
                if (noVibePill.classList.contains('active')) {
                    cont.querySelectorAll('.vibe-pill:not([data-vibe="__no_vibe__"])').forEach(p => p.classList.remove('active'));
                }
            });
            cont.appendChild(noVibePill);
            ...
            AVAILABLE_VIBES.forEach(vibe => {
                ...
                pill.addEventListener('click', () => {
                    ...
                    if (pill.classList.contains('active')) {
                        cont.querySelector('[data-vibe="__no_vibe__"]')?.classList.remove('active');
                    }
                });
    ...
    document.getElementById('btn-create-vibe-mix')?.addEventListener('click', () => {
        ...
        if (isNoVibe) {
            matchedSongs = window.globalSongsData.filter(s => !s.vibes || s.vibes.length === 0);
        } else {
            matchedSongs = window.globalSongsData.filter(song => 
                selectedVibes.every(v => song.vibes && song.vibes.includes(v))
            );
        }

        if (matchedSongs.length === 0) return alert('Keine passenden Songs gefunden.');

        const mixName = 'Vibe Mix: ' + (isNoVibe ? 'Ohne Vibe' : selectedVibes.join(', '));
        const shuffledIds = [...matchedSongs].sort(() => Math.random() - 0.5).map(s => s.id);

        const newMix = { 
            id: 'temp_' + Date.now(), 
            name: mixName, 
            cover_data: matchedSongs[0].cover_data || '', 
            songIds: shuffledIds, 
            expires: Date.now() + 86400000 
        };

        const mixes = JSON.parse(localStorage.getItem('himusic_vibe_mixes') || '[]');
        ...
        const songCount = shuffledIds.length;
        _showToast(`🎵 Vibe Mix erstellt – ${songCount} ${songCount === 1 ? 'Lied' : 'Lieder'}`, 3000);
    });
    ...
    window.openPlaylistDetails = async function(playlistId, playlistName, force = false) {
        ...
        document.querySelectorAll('.view').forEach(v => { v.classList.remove('active'); v.classList.add('hidden'); });
        viewPlaylistDetails.classList.remove('hidden'); 
        setTimeout(() => viewPlaylistDetails.classList.add('active'), 10);
        ...
        if (isTemp) {
            ...
            if(playlist) {
                const ids = playlist.songIds || (playlist.songs || []).map(s => s.id);
                const songs = ids.map(id => window._songIndex?.get(id)).filter(Boolean);
                validItems = songs.map((song, i) => ({ id: 't_'+i, song_id: song.id, sort_order: i, songs: song }));
            }
        } else {
            localStorage.setItem('himusic_last_playlist', playlistId);
            if (typeof window.renderHomeSections === 'function') window.renderHomeSections();

            playlist = window.globalPlaylistsData?.find(p => p.id === playlistId);
            if (!playlist) {
                playlistDetailsSongsContainer.innerHTML = '<div style="color:#ff3b30;text-align:center;padding:20px;">Playlist nicht gefunden. Bitte neu laden.</div>';
                return;
            }
            
            const cacheKey = `himusic_playlist_${playlistId}_songs`;
            const globalPsCache = (() => { 
                try { 
                    const r = localStorage.getItem('himusic_ps_snapshot'); 
                    return r ? JSON.parse(r) : null; 
                } catch(e) { return null; } 
            })();
            
            if (globalPsCache) {
                const playlistSongs = globalPsCache
                    .filter(ps => ps.playlist_id === playlistId)
                    .sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
                
                validItems = playlistSongs.map((ps, index) => ({
                    id: ps.id || `temp_${index}`,
                    song_id: ps.song_id,
                    sort_order: ps.sort_order || 0,
                    songs: window._songIndex?.get(ps.song_id)
                })).filter(item => item.songs);
                
                try {
                    localStorage.setItem(cacheKey, JSON.stringify(playlistSongs.map(ps => ({
                        id: ps.id,
                        song_id: ps.song_id,
                ...
                try {
                    const playlistSongs = await apiGetPlaylistSongs(playlistId);
                    validItems = playlistSongs.map((song, index) => ({
                        id: song.id, 
                        song_id: song.id,
                        sort_order: index,
                        songs: song
                    ...
                    playlistDetailsSongsContainer.innerHTML = `<div style="color:#ff3b30;text-align:center;padding:20px;">Fehler: ${e.message}</div>`;
                    return;
                }
            }
        }
        ...
        const renderPlaylistDetailsUI = () => {
            ...
            if (playlist && playlist.cover_data && playlist.cover_data.length > 10) { 
                coverDiv.style.backgroundImage = `url('${playlist.cover_data}')`; 
                coverDiv.innerHTML = '';
            } else { 
                coverDiv.style.backgroundImage = 'none'; 
                coverDiv.innerHTML = `<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.7)" stroke-width="2" style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);"><line x1="8" y1="6" x2="21" y2="6"></line><line x1="8" y1="12" x2="21" y2="12"></line><line x1="8" y1="18" x2="21" y2="18"></line></svg>`; 
            ...
            if (validItems.length === 0) { 
                playlistDetailsSongsContainer.innerHTML = '<div style="text-align:center; padding: 40px 20px; color: var(--text-secondary);">Diese Playlist ist leer.</div>'; 
                return; 
            }
            ...
            function renderPlaylistBatch() {
                const end = Math.min(plRendered + PL_BATCH, validItems.length);
                const frag = document.createDocumentFragment();
                for (let i = plRendered; i < end; i++) {
                    const item = validItems[i];
                    const songDiv = document.createElement('div');
                    songDiv.className = 'song-item';
                    songDiv.dataset.id = item.song_id;
                    updateSongDOM(songDiv, item.songs, item.id);
                    frag.appendChild(songDiv);
                }
                if (plSentinel && plSentinel.parentNode) plSentinel.remove();
                playlistDetailsSongsContainer.appendChild(frag);
                plRendered = end;

                if (plRendered < validItems.length) {
                    plSentinel = document.createElement('div');
                    plSentinel.style.height = '1px';
                    playlistDetailsSongsContainer.appendChild(plSentinel);
                    if (plObserver) plObserver.disconnect();
                    plObserver = new IntersectionObserver(entries => {
                        if (entries[0].isIntersecting) renderPlaylistBatch();
                    }, { rootMargin: '400px' });
                    plObserver.observe(plSentinel);
                } else {
                    if (plObserver) { plObserver.disconnect(); plObserver = null; }
                }
            }
            renderPlaylistBatch();
                ...
                onEnd: async function () {
                    ...
                    if (isTemp) {
                        ...
                        const updates = Array.from(items).map((item, index) => ({
                            song_id: parseInt(item.dataset.id), 
                            sort_order: index
                        }));
    ...
    document.getElementById('btn-pld-play')?.addEventListener('click', () => {
        ...
        window.playSong(first.title, first.artist, first.cover_data, first.file_url);
        window.playbackQueue = window.currentPlaylistSongs.slice(1);
        savePlayerState();
    });
    ...
    document.getElementById('btn-shuffle')?.addEventListener('click', (e) => {
        ...
        if(isShuffle) {
            window.playbackQueue = window.playbackQueue.sort(() => 0.5 - Math.random());
        }
    });
    ...
    if (audioPlayer) {
        let _errRetry = 0;
        let _errTimer = null;
        const _ERR = { 1:'Abgebrochen', 2:'Netzwerk-Fehler', 3:'Decode-Fehler', 4:'URL nicht erreichbar' };

        audioPlayer.addEventListener('error', () => {
            const err = audioPlayer.error;
            const code = err ? err.code : 0;
            const label = _ERR[code] || 'Unbekannt';
            console.error('Audio Fehler Code ' + code + ' (' + label + ')');
            isChangingSong = false;

            if (code === 4) {
                _errRetry = 0;
                _showToast('Song nicht erreichbar (Fehler 4)<br><small style="opacity:0.6">Prüfe Verbindung zu Cloudflare</small>');
                return;
            }
            ...
            if (_errRetry < 1) {
                ...
                _errTimer = setTimeout(() => {
                    console.log('play() retry nach Netzwerk-Fehler...');
                    audioPlayer.play().catch(() => {});
                }, 3000);
            } else {
                _errRetry = 0;
                _showToast('Song konnte nicht geladen werden<br><small style="opacity:0.6">Fehler ' + code + ': ' + label + '</small>');
            }
        });
    ...
    function buildQueueItem(song, type) {
        ...
        const coverSrc = song.coverUrl || song.cover_data || '';
        const cover = coverSrc.length > 10 ? `url('${coverSrc}')` : 'var(--accent)';

        if (type === 'current') {
            div.classList.add('playing-active');
            div.style.cssText = 'background:rgba(250,35,59,0.15);border:1px solid var(--accent);border-radius:10px;padding:10px;margin:6px 0 12px;';
            div.innerHTML = `
                <div class="song-cover" style="background:${cover};background-size:cover;position:relative;">
                    <div class="playing-anim" style="position:absolute;bottom:5px;right:5px;transform:scale(0.6);"><span></span><span></span><span></span></div>
                </div>
                <div class="song-info">
                    <div class="song-title" style="color:var(--accent);font-size:16px;">${song.title}</div>
                    <div class="song-artist">${song.artist}</div>
                </div>
                <div style="font-size:10px;color:var(--accent);font-weight:700;letter-spacing:1px;flex-shrink:0;">LÄUFT</div>`;
            ...
            div.innerHTML = `
                ...
                <div style="font-size:10px;color:var(--text-secondary);flex-shrink:0;padding-right:4px;">DAVOR</div>`;
            div.addEventListener('click', () => {
                const idx = window.playbackHistory.indexOf(song);
                if (idx === -1) return;
                const songsAfter = window.playbackHistory.splice(idx + 1);
                if (window.currentSongData) songsAfter.push(window.currentSongData);
                window.playbackQueue = [...songsAfter.reverse(), ...window.playbackQueue];
                window.playbackHistory.splice(idx, 1); 
                window.currentContextSongId = song.id || window.currentContextSongId;
                _skipNextHistoryPush = true; 
                window.playSong(song.title, song.artist, song.cover_data || song.coverUrl, song.file_url || song.fileUrl);
            ...
            div.innerHTML = `
                ...
                <div class="drag-handle" style="display:block;flex-shrink:0;">≡</div>`;
            ...
            div.addEventListener('touchend', (e) => {
                ...
                if (qStartX - e.changedTouches[0].clientX > 50) {
                    const actualIdx = window.playbackQueue.indexOf(song);
                    if (actualIdx > -1) window.playbackQueue.splice(actualIdx, 1);
            ...
            div.addEventListener('click', (e) => {
                ...
                const idx = window.playbackQueue.indexOf(song);
                if (idx === -1) return;
                const skipped = window.playbackQueue.splice(0, idx);
                if (window.currentSongData) {
                    window.playbackHistory.push(window.currentSongData);
                    skipped.forEach(s => window.playbackHistory.push(s));
                }
                window.playbackQueue.shift(); 
                window.currentContextSongId = song.id || window.currentContextSongId;
                _skipNextHistoryPush = true; 
                window.playSong(song.title, song.artist, song.cover_data || song.coverUrl, song.file_url || song.fileUrl);
                document.getElementById('queue-overlay').classList.remove('active');
    ...
    document.getElementById('btn-queue-menu')?.addEventListener('click', () => {
        ...
        const historyToShow = window.playbackHistory.slice(-10).reverse(); 
        if (historyToShow.length > 0) {
            ...
            histLabel.innerText = 'Verlauf';
            qContainer.appendChild(histLabel);
            historyToShow.forEach(song => qContainer.appendChild(buildQueueItem(song, 'history')));
        }
        ...
        if (window.currentSongData) {
            ...
            nowLabel.innerText = 'Läuft jetzt';
            qContainer.appendChild(nowLabel);
            qContainer.appendChild(buildQueueItem(window.currentSongData, 'current'));
        }

        if (window.playbackQueue.length === 0) {
            ...
            nextLabel.innerText = 'Als nächstes';
            qContainer.appendChild(nextLabel);

            const nextContainer = document.createElement('div');
            nextContainer.id = 'queue-next-container';
            window.playbackQueue.slice(0, 50).forEach(song => nextContainer.appendChild(buildQueueItem(song, 'next')));
            qContainer.appendChild(nextContainer);

            if (window.playbackQueue.length > 50) {
                const moreMsg = document.createElement('div');
                moreMsg.style.cssText = 'padding:15px;text-align:center;font-size:12px;color:var(--text-secondary);';
                moreMsg.innerText = `+ ${window.playbackQueue.length - 50} weitere Lieder...`;
                qContainer.appendChild(moreMsg);
            }
            ...
            queueSortable = new Sortable(nextContainer, {
                ...
                onEnd: function(evt) {
                    const movedItem = window.playbackQueue.splice(evt.oldIndex, 1)[0];
                    window.playbackQueue.splice(evt.newIndex, 0, movedItem);
                    savePlayerState();
                }
            });
        }
    ...
    document.getElementById('btn-audio-out')?.addEventListener('click', async () => {
        ...
        try {
            if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices || !audioPlayer.setSinkId) {
                outList.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--text-secondary);">Dein Browser unterstützt diese Funktion leider nicht (nur Chrome/Edge PC).</div>';
            } else {
    ...
    if (hdAudioToggle) {
        hdAudioToggle.addEventListener('change', (e) => {
            if (e.target.checked) {
                ...
                alert("HD Audio aktiviert! Maximale Qualität & Lautstärke geladen.");
            }
        });
    }
    ...
    document.getElementById('bp-ctx-add-playlist')?.addEventListener('click', () => {
        ...
        selectedSongs.clear(); selectedSongs.add(String(window.currentContextSongId));
        window.openPlaylistSelection();
    });
    ...
    document.getElementById('bp-ctx-create-station')?.addEventListener('click', () => {
        ...
        const song = window._songIndex?.get(parseInt(activeId));
        if (!song) return;
    ...
    if (colorPicker) {
        const savedColor = localStorage.getItem('himusic_theme_color');
        if (savedColor) {
        ...
        colorPicker.addEventListener('input', (e) => {
            ...
            localStorage.setItem('himusic_theme_color', newColor);
            if(typeof window.updateActiveHighlights === 'function') window.updateActiveHighlights();
        });
    }
    ...
    window.updateAppStats = function() {
        ...
        if (statsEl) {
            const songCount = (typeof window.globalSongsData !== 'undefined') ? window.globalSongsData.length : 0;
    ...
    document.getElementById('btn-backup-download')?.addEventListener('click', () => {
        const backupData = {
            state: JSON.parse(localStorage.getItem('himusic_state') || '{}'),
            mixes: JSON.parse(localStorage.getItem('himusic_vibe_mixes') || '[]'),
            stations: JSON.parse(localStorage.getItem('himusic_stations') || '[]'),
            theme: localStorage.getItem('himusic_theme_color') || '#fa233b',
        ...
        a.href = url;
        a.download = `Himusic_Backup_${new Date().toISOString().split('T')[0]}.json`;
    ...
    document.getElementById('btn-carplay')?.addEventListener('click', () => {
        alert("🚗 Apple CarPlay & Android Auto bereit!\n\nVerbinde dein Handy einfach per Kabel oder Bluetooth mit deinem Auto. Da Himusic jetzt die native Media-Schnittstelle nutzt, werden Songs, Cover und die Steuerung automatisch auf dein Auto-Display übertragen!");
    });
    ...
    document.querySelectorAll('.action-sheet-overlay').forEach(overlay => {
        ...
        if(!sheet) return;
        let sheetStartY = 0;
        let swipeStartedInScrollable = false;
        ...
        sheet.addEventListener('touchstart', (e) => {
            ...
            swipeStartedInScrollable = !!scrollable;
        }, {passive: true});
