...
All client-side calls to this Worker go through `_apiFetch()` (app2.js, drop-in `fetch()` replacement) rather than raw `fetch()` — use it for any new call to `${API_URL}`. Calls to third-party hosts (iTunes search, etc.) must stay plain `fetch()`. **The `X-Api-Key` header injection inside `_apiFetch()` is currently commented out** (see ADR-005's "Korrektur" entry): sending it against the still-unupdated live Worker triggered a CORS preflight that the Worker's `Access-Control-Allow-Headers` didn't permit, which silently broke *every* API call (saving tags, background sync, everything) app-wide. Do not re-enable that line until the new Worker code (with `X-Api-Key` in its CORS allow-list and the actual auth check) is confirmed deployed — check ADR-005's status before touching this.
