...
All client-side calls to this Worker go through `_apiFetch()` (app2.js, drop-in `fetch()` replacement, sends `X-Api-Key: <config.js apiKey>` on every call) rather than raw `fetch()` — use it for any new call to `${API_URL}`. Calls to third-party hosts (iTunes search, etc.) must stay plain `fetch()`, never `_apiFetch()`, so the key is never sent to them. **As of 2026-07-11 the Worker enforces this on every route except `/media/*` (GET) and `/internal/register`** — verified live via curl (401 without the header, 200 with it). See ADR-005 for the full rollout story, including three bugs hit along the way (CORS preflight block, stale Service-Worker cache, a trailing-newline secret) — useful context if API calls ever start failing again after a similar change.

### Authentication
Two independent layers, don't confuse them:
1. **App login** (`login.html`): a hardcoded password gates a `localStorage.getItem('himusic_auth') === 'true'` flag, client-side only, no server validation. Given the public-repo/GitHub-Pages hosting above, this provides **no real protection** — anyone can read the password directly from the repo. UX speed-bump only.
2. **Worker API auth** (`X-Api-Key` header, see above): actually enforced server-side as of 2026-07-11 (ADR-005). Stops automated/casual access to the bare Worker URL. Does **not** stop someone who reads the public repo's `config.js` and copies the key from there — that gap only closes once the pending hosting migration (private hosting, see ADR-005 "Next Steps") lands.
