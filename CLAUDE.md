...
### Video-Import (Audio-Extraktion) — ADR-014, hinzugefügt 2026-09-20
Wählt der Nutzer beim Datei-Import (`btn-add-songs` → `native-file-upload`, `accept` deckt jetzt `video/*` inkl. `.mov`/`.m4v` ab, nicht nur `.mp4`) ein Video statt einer Audiodatei, landet **nicht** das Video selbst als "Song" in der Bibliothek — `_runFileImportBatch`s `uploadOne()` erkennt Videos an MIME-Typ **oder** Endung (`_isVideoFile()`) und schickt sie zuerst durch `_nativeExtractAudio(file)`.

**Nativ, nicht in JS** — anders als beim YouTube-Import (dort ein Format, das JS selbst remuxt) kann ein Video vom iPhone in beliebigen Containern/Codecs vorliegen (HEVC/.mov, H.264/.mp4, …). Ein eigener JS-Parser dafür wäre kein vertretbarer Aufwand; AVFoundation kennt jedes Format nativ. Neuer WKScriptMessageHandler-Kanal `himusicMedia` (WebShellView.swift, parallel zu `himusicHttp`) nimmt `{cmd:"extractAudio", dataBase64, extension}` entgegen, `AudioExtractor.swift` schreibt die Bytes in eine Temp-Datei, lädt die Audiospur (`AVURLAsset.loadTracks(withMediaType:.audio)` — wirft `noAudioTrack`, falls keine da ist), exportiert sie mit `AVAssetExportPresetAppleM4A` (ältere `exportAsynchronously`-API, nicht die neuere `export(to:as:)` — Deployment-Target ist iOS 16, Letztere braucht iOS 18) und gibt `{dataBase64, durationSeconds}` zurück. `durationSeconds` kommt damit als **echte, gemessene** Dauer zurück — ein Bonus gegenüber dem normalen Datei-Upload, der bisher immer `duration: 0` speicherte.

**Größengrenze:** `AudioExtractor.maxInputBytes` (Swift) und `_VIDEO_EXTRACT_MAX_BYTES` (JS, `app2.js`) sind beide `300 * 1024 * 1024` — **müssen synchron gehalten werden**, ändert sich einer, den anderen mitziehen. Der Grund liegt an der Bridge selbst: die Datei geht als kompletter Base64-String durch `WKScriptMessageHandler` (kein Chunking/Streaming), das belastet den Speicher des Webinhalts-Prozesses spürbar (~1,35× Rohgröße als JS-String) — zu groß riskiert einen stillen Absturz des Webinhalts-Prozesses. JS prüft die Größe *vor* dem Einlesen/Encodieren, damit ein zu großes Video gar nicht erst durch `_bytesToB64()` (Blockweise, 32 KB, gegen Call-Stack-Sprengung bei `String.fromCharCode.apply`) läuft.

**Nur in der Hülle.** Ohne `himusicMedia`-Bridge (reiner Browser) wirft `_nativeExtractAudio` sofort einen klaren Fehler — kein stiller Fallback aufs Roh-Video-Hochladen.

Verifiziert mit Playwright gegen eine gefälschte Bridge: Video wird erkannt, Datei geht **nicht** roh raus (Upload-Blob-Größe = extrahiertes Audio, nicht Video-Größe), Dateiname wird zu `.m4a` umbenannt, echte Dauer landet im `POST /songs`-Body, normale Audiodateien bleiben unverändert (Bridge wird für sie nie aufgerufen), Größenlimit blockt *vor* dem Bridge-Aufruf mit lesbarer Meldung, fehlende Bridge scheitert klar statt still das Video hochzuladen. Die Swift-Seite selbst (AVFoundation-Export, echtes Gerät) konnte nicht getestet werden — kein Build-Toolchain dafür hier, nur `swiftc -parse` auf Syntaxfehler geprüft.
